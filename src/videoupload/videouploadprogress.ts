 ...
 */
    public init(): void {
        ...
        if (editor.plugins.has('VideoBlockEditing')) {
            editor.editing.downcastDispatcher.on<DowncastAttributeEvent>('attribute:uploadStatus:videoBlock', (...args) => this.uploadStatusChange(...args));
        }

        if (editor.plugins.has('VideoInlineEditing')) {
            editor.editing.downcastDispatcher.on<DowncastAttributeEvent>('attribute:uploadStatus:videoInline', (...args) => this.uploadStatusChange(...args));
        }
    }
    ...
    private uploadStatusChange: GetCallback<DowncastAttributeEvent> = (evt, data, conversionApi) => {
        ...
        const viewFigure = editor.editing.mapper.toViewElement(modelVideo)! as ViewContainerElement;;
...
function _showPlaceholder(videoUtils: VideoUtils, placeholder: string, viewFigure: ViewContainerElement, writer: DowncastWriter) {
...
function _getUIElement(videoFigure: ViewElement, uniqueProperty: string): ViewUIElement | undefined {
...
function _displayLocalVideo(videoUtils: VideoUtils, viewFigure: ViewElement, writer: DowncastWriter, loader: FileLoader) {
    if (loader.data) {
        const viewVideo = videoUtils.findViewVideoElement(viewFigure)!;
