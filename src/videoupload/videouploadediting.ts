...
import { UpcastWriter, type Element, type Item, type Writer, type DataTransfer, type ViewElement } from '@ckeditor/ckeditor5-engine';
...
export default class VideoUploadEditing extends Plugin {
    ...
    public static get requires() {
        return [FileRepository, Notification, ClipboardPipeline, VideoUtils] as const;
    }
    ...
    public init(): void {
        ...
        const videoTypes = createVideoTypeRegExp(editor.config.get('video.upload.types')! as string[]);
        ...
        this.listenTo(clipboardPipeline, 'inputTransformation', (evt, data) => {
            ...
            const fetchableVideos = Array.from(editor.editing.view.createRangeIn(data.content))
                ...
                .filter(viewElement => isLocalVideo(videoUtils, viewElement) && !viewElement.getAttribute('uploadProcessed'))
                .map(viewElement => { return { promise: fetchLocalVideo(viewElement), videoElement: viewElement }; });
            ...
            for (const entry of changes) {
                if (entry.type === 'insert' && entry.name !== '$text') {
                    ...
                    for (const videoElement of getVideosFromChangeItem(editor, item)) {
                        const uploadId = videoElement.getAttribute('uploadId') as string;
                        ...
                        if (isInsertedInGraveyard) {
                            ...
                            this._uploadVideoElements.set(uploadId, videoElement as Element);
        ...
        this.on<VideoUploadCompleteEvent>('uploadComplete', (evt, { videoElement, data }) => {
        ...
        return loader.read()
            .then(() => {
                ...
                if (env.isSafari) {
                    ...
                    editor.editing.view.once('render', () => {
                        ...
                        const domFigure = editor.editing.view.domConverter.mapViewToDom(viewVideo.parent) as HTMLElement | undefined;
            ...
            .then(data => {
                model.enqueueChange({ isUndoable: false }, writer => {
                    ...
                    this.fire<VideoUploadCompleteEvent>('uploadComplete', { data, videoElement });
                });
