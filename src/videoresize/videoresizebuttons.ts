...
import { ButtonView, DropdownButtonView, Model, createDropdown, addListToDropdown, type ListDropdownItemDefinition } from '@ckeditor/ckeditor5-ui';
...
export default class VideoResizeButtons extends Plugin {
	...
	constructor(editor: Editor) {
		...
		this._resizeUnit = editor.config.get('video.resizeUnit') as 'px' | '%' | undefined;
	}
	...
	public init(): void {
		...
		const options: Array<VideoResizeOption> = editor.config.get('video.resizeOptions')! as Array<VideoResizeOption>;
		const command: ResizeVideoCommand = editor.commands.get('resizeVideo') as ResizeVideoCommand;
	...
	private _registerVideoResizeDropdown(options: Array<VideoResizeOption>): void {
		const editor = this.editor;
		const t = editor.t;
	...
	private _getOptionLabelValue(option: VideoResizeOption, forTooltip: boolean = false): string {
	...
	private _getResizeDropdownListItemDefinitions(options: Array<VideoResizeOption>,
		command: ResizeVideoCommand): Collection<ListDropdownItemDefinition>  {
