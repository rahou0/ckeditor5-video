...
export default class VideoStyleUI extends Plugin {
    ...
    public init(): void {
        ...
        const toolbarConfig = this.editor.config.get('video.toolbar') as Array<VideoStyleDropdownDefinition> || [];
        const videoStyleEditing: VideoStyleEditing = plugins.get('VideoStyleEditing') as VideoStyleEditing;
        ...
        const definedDropdowns = translateStyles(
            [...toolbarConfig.filter(isObject) as Array<VideoStyleDropdownDefinition>, ...utils.getDefaultDropdownDefinitions(plugins)],
            this.localizedDefaultStylesTitles
        );
    ...
    private _createDropdown(dropdownConfig: VideoStyleDropdownDefinition, definedStyles: Array<VideoStyleOptionDefinition>): void {
    ...
    private _createButton(buttonConfig: VideoStyleOptionDefinition): void {
        ...
        this.editor.ui.componentFactory.add(getUIComponentName(buttonName), locale => {
            const command: VideoStyleCommand = this.editor.commands.get('videoStyle') as VideoStyleCommand;
...
function translateStyles<T extends VideoStyleOptionDefinition | VideoStyleDropdownDefinition>(styles: Array<T>, titles: Record<string, string>): Array<T> {
