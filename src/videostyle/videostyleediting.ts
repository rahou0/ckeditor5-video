...
export default class VideoStyleEditing extends Plugin {
    ...
    public init(): void {
        ...
        editor.commands.add('videoStyle', new VideoStyleCommand(editor, this.normalizedStyles as Array<VideoStyleOptionDefinition>));
    }
        ...
        document.registerPostFixer(writer => {
            ...
            for (const change of document.differ.getChanges()) {
                if (change.type === 'insert' || change.type === 'attribute' && change.attributeKey === 'videoStyle') {
                    ...
                    if (!videoStyleDefinition || !videoStyleDefinition.modelElements.includes((element as Element).name)) {
                        writer.removeAttribute('videoStyle', element as Element);
                        changed = true;
                    }
                }
            }
