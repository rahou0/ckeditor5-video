...
export function modelToViewStyleAttribute(styles: Array<VideoStyleOptionDefinition>): GetCallback<DowncastAttributeEvent> {
...
export function viewToModelStyleAttribute(styles: Array<VideoStyleOptionDefinition>): GetCallback<UpcastElementEvent> {
  const nonDefaultStyles: Record<string, Array<VideoStyleOptionDefinition>> = {
    videoInline: styles.filter(
      (style) => !style.isDefault && style.modelElements.includes("videoInline")
    ),
    videoBlock: styles.filter(
      (style) => !style.isDefault && style.modelElements.includes("videoBlock")
    ),
  };
  ...
  return (evt, data, conversionApi) => {
    ...
    if (nonDefaultStyles[(modelVideoElement as Element).name])
      for (const style of nonDefaultStyles[(modelVideoElement as Element).name]) {
        if (
          style &&
...
function getStyleDefinitionByName(name: string, styles: Array<VideoStyleOptionDefinition>): VideoStyleOptionDefinition | undefined  {
