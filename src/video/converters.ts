...
export function downcastVideoAttribute(videoUtils: VideoUtils, videoType: 'videoBlock' | 'videoInline', attributeKey: string): (dispatcher: DowncastDispatcher) => void {
	const converter: GetCallback<DowncastAttributeEvent<Element>> = (evt, data, conversionApi) => {
		...
		writer.setAttribute(data.attributeKey, data.attributeNewValue || '', video);

	};

	return dispatcher => {
		dispatcher.on<DowncastAttributeEvent<Element>>(`attribute:${attributeKey}:${videoType}`, converter);
	};
