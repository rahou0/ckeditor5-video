...
import { ButtonView, View, SplitButtonView, ViewCollection, submitHandler, createDropdown, FocusCycler, type InputTextView, type LabeledFieldView, DropdownView } from '@ckeditor/ckeditor5-ui';
import { Collection, FocusTracker, KeystrokeHandler, type Locale } from '@ckeditor/ckeditor5-utils';
 ...
 */
     ...
     * @observable
     */
    declare public videoURLInputValue: string;
            ...
            for (const [integration, integrationView] of Object.entries(integrations)) {
                if (integration === 'insertVideoViaUrl') {
                    (integrationView as LabeledFieldView<InputTextView>).
                        fieldView.bind('value').to(this, 'videoURLInputValue', (value:string) => value || '');

                    (integrationView as LabeledFieldView<InputTextView>).fieldView.on('input', () => {
                        this.videoURLInputValue = (integrationView as LabeledFieldView<InputTextView>).fieldView.element!.value.trim();
                    });
                }
...
_createActionButtons(locale: Locale): { insertButtonView: ButtonView; cancelButtonView: ButtonView } {
        const t = locale.t;
